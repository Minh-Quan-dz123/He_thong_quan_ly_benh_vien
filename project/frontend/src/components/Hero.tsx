    ...
    const fetchNews = async () => {
      try {
        ...
        const parsed: Article[] = items.map((item) => {
          ...
          const image = imgMatch ? imgMatch[1] : 'https://images.unsplash.com/photo-1505751172876-fa1923c5c528?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80';
        ...
        // Shuffle and take 5 random articles from the latest news
        const shuffled = parsed.sort(() => 0.5 - Math.random()).slice(0, 5);
        if (shuffled.length > 0) {
          setArticles(shuffled);
        }
      } catch (e) {
        console.error('Failed to fetch news for hero', e);
      }
    };
