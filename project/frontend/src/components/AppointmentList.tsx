...
const AppointmentList = ({ currentUser, showToastMsg }: AppointmentListProps) => {
  ...
  error;// thêm vào để tránh lỗi
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/patient/appointments/patient/${currentUser.id}`, {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/patient/appointments/${selectedAppointmentId}`, {
        method: 'DELETE',
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
