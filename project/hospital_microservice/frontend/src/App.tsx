...
import AdminGuideManagement from './components/AdminGuideManagement';
...
function App() {
  ...
  useEffect(() => {
    ...
    setIsAdminSignInOpen(path === '/admin/login');
  ...
  const handleCloseModals = () => {
    ...
    if (['/login', '/admin/login', '/register', '/forgot-password'].includes(location.pathname)) {
      navigate('/');
    }
  };
  ...
  const handleLogin = (user: any, role: 'patient' | 'doctor' | 'admin') => {
    setIsLoggedIn(true);
    setCurrentUser(user);
    setUserRole(role);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-16">      <Toaster position="top-right" reverseOrder={false} />      <style>{`
